...
export default function QuestForm({ quest, handleQuestChange, handleLogoUpload, fileInputRef, togglePublish }) {
    return (
                ...
                <div className="w-full">
                            ...
                            <div className="flex-shrink-0">
                                {quest.logoPreview ? (
                                    <div className="relative w-24 h-24 bg-zinc-700 rounded-md overflow-hidden">
                                        <img
                                            src={quest.logoPreview || "/placeholder.svg"}
                    ...
                    <div className="flex items-center mt-4">
                        <button
                            ...
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-amber-600 focus:ring-offset-2 ${quest.published ? "bg-amber-600" : "bg-zinc-500"
                                }`}
                        >
                            <span
                                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${quest.published ? "translate-x-6" : "translate-x-1"
                                    }`}
                            />
                        </button>
                        <span className="ml-3 text-sm">{quest.published ? "Published" : "Unpublished"}</span>
                    </div>
                </div>
            </div>
        </div>
